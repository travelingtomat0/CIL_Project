...
def run(embedding_size=12):
    ...
    with open("output/bias_ae_cv.txt", "w") as f:
        rmses = []
        for i in range(10):
            print(f"{i + 1} / 10:")
            ...
            model = keras.models.load_model("temp/bias_ae.hdf5",
    ...
    util.export_matrix_to_csv(predictions, "output/bias_ae_train.csv", export_train=True)
    util.export_matrix_to_csv(predictions, "output/bias_ae_sub.csv", export_submission=True, clip=True)
